...
test('numbers', function(t) {
  ...
  t.deepEquals(
    numbers(['cat', 1, 23.9, '33.2']),
    [1, 23.9, 33.2],
    'removes non-nums, converts str numbers to numers'
  );
...
test('mode', function(t) {
  ...
  t.deepEquals(
    mode([1, 1, 7, 5, 5, 8, 7]),
    new Set([1, 7, 5]),
    "multi-modal works and order doesn't matter (yay Set)"
  );

  t.deepEquals(
    mode([1, '6', 2, 8, 7, 2]),
    2,
    'mode works with stringification'
  );
...
test('percentile', function(t) {
  ...
  t.deepEquals(
    percentile([3, 5, 7, 8, 9, 11, 13, 15], 0.25),
    6,
    'percentile works'
  );
...
test('quartiles', t => {
  ...
  t.equals(typeof quartiles, 'function', 'percentile is a function');
  ...
  t.deepEquals(
    quartiles(scores)[1],
    stats.median(scores),
    'Second quartile equals median'
  );

  t.deepEquals(
    quartiles(scores)[0],
    percentile(scores, 0.25),
    'first quartile works'
  );

  t.deepEquals(quartiles(scores)[0], 5, 'first quartile works');
  t.deepEquals(quartiles(scores)[2], 9, 'last quartile works');

  t.deepEquals(
    quartiles(scores)[2],
    percentile(scores, 0.75),
    'third quartiles works'
  );

  t.end();
});

test('interquartile', t => {
  var scores = [4, 4, 5, 5, 5, 5, 6, 6, 6, 7, 7, 7, 8, 8, 9, 9, 9, 10, 10, 10];
  var quartiles = stats.quartiles;
  var interquartile = stats.interquartile;

  t.deepEquals(interquartile(scores), 4, 'Interquartile works');

  t.end();
});

test('outliersBounds', t => {
  var scores = [4, 4, 5, 5, 5, 5, 6, 6, 6, 7, 7, 7, 8, 8, 9, 9, 9, 10, 10, 10];
  var outliersBounds = stats.outliersBounds;

  t.deepEquals(outliersBounds(scores), [-1, 15], 'Boundaries ok');

  t.end();
});

test('withoutOutliers', t => {
  var scores = [4, 4, 5, 5, 5, 5, 6, 6, 6, 7, 7, 7, 8, 8, 9, 9, 9, 10, 10, 10];
  var scoresWith = [
    -2,
    4,
  ...
  var removeOutliers = stats.removeOutliers;

  t.deepEquals(removeOutliers(scores), scores, 'No outliers to remove ok');
  t.deepEquals(
    removeOutliers(scoresWith),
    [4, 5, 5, 5, 5, 6, 6, 6, 7, 7, 7, 8, 8, 9, 9, 9, 10],
...
test('outliers', t => {
  ...
  t.deepEquals(outliers(scores), [], 'No outliers ok');
  t.deepEquals(outliers(scoresWith), [-2, 25, 26], 'Some outliers');
...
test('histogram', t => {
  ...
  t.deepEquals(
    histogram([-100, -53, 10, 100, -22, 0, 0, 1, 3, 44], 20),
    expect,
    'range goes from negative to positive'
  );
